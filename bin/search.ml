...
let ensure_dir : string -> unit Deferred.t =
 fun dir ->
  match%bind Sys.file_exists dir with
  | `No -> try_with (fun () -> Unix.mkdir dir) |> Deferred.ignore
  | _ -> return ()

let in_dir : string -> f:(unit -> 'a Deferred.t) -> 'a Deferred.t =
 fun dir ~f ->
...
let validate : Ralgebra.t -> bool =
 fun r ->
  ...
  let types_check =
    List.for_all (Ralgebra.layouts r) ~f:(fun l ->
        try
...
let main :
    ...
    -> int
    -> unit Deferred.t =
 fun ?num ?sample ?max_time ?max_disk ?max_size ~debug ~queue ~db
     {name; sql; query; params} dir id ->
  ...
  ( try
      Db.exec qconn ~params:[name]
        "create table if not exists $0 (like template including all)"
      |> ignore
    with _ -> () ) ;
  let serialize : Candidate.t -> unit Or_error.t Deferred.t =
   fun cand ->
    ...
    let%bind do_write =
      ...
      let query =
        Db.subst_params params
          "insert into $0 (hash,program_size,valid,search_state,search_transforms) \
    ...
    if do_write then
      ...
      |> Pervasives.ignore
  in
  let deserialize : string -> Candidate.t Or_error.t Deferred.t =
   fun hash ->
    let open Candidate.Binable in
    ...
    >>| Result.join
    >>| fun e ->
    Logs.info (fun m -> m "Done loading candidate %s." hash) ;
    e
  in
  let with_candidate : (string -> unit Or_error.t Deferred.t) -> unit Deferred.t =
   fun f ->
    (* Grab a candidate from the frontier. *)
    Db.exec1 qconn ~params:[queue] {| begin; lock table $0 in exclusive mode; |}
    |> ignore ;
    let hash =
      Db.exec1 qconn ~params:[queue]
        {|
...
|}
    in
    Db.exec1 qconn ~params:[queue] {| commit; |} |> ignore ;
    match hash with
  ...
  let is_done : unit -> string option =
   fun () ->
    ...
    let check_disk () =
      match max_disk with
      | Some b -> (
        match Db.exec1 qconn ~params:[queue] "select sum(program_size) from $0" with
      ...
      with_candidate (fun hash ->
          ...
          List.iter ~how:`Sequential Transform.transforms ~f:(fun t ->
              ...
              List.iter ~how:`Sequential (Candidate.run tf r) ~f:(fun r ->
                  ...
                  let name =
                    sprintf "child %s of %s"
                      (Candidate.transforms_to_string cand.transforms)
                      hash
                  in
...
let () =
  ...
  async ~summary:"Generate candidates from a benchmark file."
    (let%map_open db =
     ...
     and num =
       flag "num" ~aliases:["n"] (optional int)
         ~doc:"N the number of candidates to enumerate"
     and max_time =
       flag "max-time" ~aliases:["mt"] (optional int)
         ~doc:"SEC the maximum amount of time to use"
     and max_disk =
       flag "max-disk" ~aliases:["md"] (optional int)
         ~doc:"BYTES the maximum amount of disk space to use"
     and max_size =
       flag "max-size" ~aliases:["ms"] (optional int)
         ~doc:"BYTES the maximum candidate size (serialized)"
     and bench = anon ("bench" %: bench)
     and dir = anon ("dir" %: file)
     and id = anon ("id" %: int) in
     fun () ->
