...
module PredCtx = struct
  module Key = struct
    module T = struct
      type t = Field of Field.t | Var of TypedName.NameOnly.t | Dummy
  ...
  let of_tuple : Tuple.t -> t =
   fun t ->
    List.fold_left t
      ...
      ~f:(fun m v -> Map.set m ~key:(Field v.field) ~data:v.value)

  let of_vars : (string * primvalue) list -> t =
   fun l ->
    List.fold_left l
      ~init:(Map.empty (module Key))
      ~f:(fun m (k, v) -> Map.set m ~key:(Var (k, PrimType.of_primvalue v)) ~data:v)
end
...
module ValueMap = struct
  module Elem = struct
    ...
    let of_primvalue : primvalue -> t =
     fun p -> {value= p; rel= Relation.dummy; field= Field.dummy}
  end
...
type ordered_list = {field: Field.t; order: [`Asc | `Desc]; lookup: Range.t}
[@@deriving compare, sexp, hash, bin_io]

type table = {field: Field.t; lookup: PredCtx.Key.t}
[@@deriving compare, sexp, hash, bin_io]

type grouping = {key: Field.t list; output: Field.t Ralgebra0.agg list}
[@@deriving compare, sexp, hash, bin_io]

type scalar_node = {rel: Relation.t; field: Field.t}
[@@deriving compare, sexp, hash, bin_io]
...
let compare_scalar : scalar -> scalar -> int = fun s1 s2 -> compare s1.tag s2.tag

let scalar_of_sexp : Sexp.t -> scalar =
 fun s ->
  let n = [%of_sexp : scalar_node] s in
  scalar n.rel n.field

let sexp_of_scalar : scalar -> Sexp.t = fun s -> [%sexp_of : scalar_node] s.node

let hash_scalar {hkey; _} = hkey
...
module Binable = struct
  ...
  type t = {layout: binable_layout; schema: binable_schema}
  [@@deriving sexp, bin_io, hash]

  let binable_schema_of_layout : layout -> binable_schema =
   fun l ->
    let rec f : layout -> binable_schema =
     fun l ->
      match l.node with
      ...
      | Grouping (ls, _) ->
          let x1, x2 =
            List.map ls ~f:(fun (x1, x2) -> (f x1, f x2)) |> List.all_equal_exn
          in
          GroupingS (x1, x2)
      | Empty -> EmptyS
    in
  ...
  let binable_layout_of_layout : layout -> binable_layout =
   fun l ->
  ...
  let of_layout : layout -> t =
   fun l -> {layout= binable_layout_of_layout l; schema= binable_schema_of_layout l}
    ...
    let rec f l s =
      ...
      | Table (xs, m), TableS (scalar, s) ->
          table
            ( List.map
                ~f:(fun (k, v) ->
                  (Value.{rel= scalar.rel; field= scalar.field; value= k}, f v s) )
                xs
      ...
      | _ ->
          Error.create "Mismatched layout & schema." (l, s)
            [%sexp_of : binable_layout * binable_schema]
          |> Error.raise
    in
    f layout schema
end
...
let grouping_to_schema : Field.t Ralgebra0.agg list -> Schema.t =
 fun output ->
  List.map output ~f:(function
    | Count -> Field.{fname= "count"; dtype= DInt}
    | Key f -> f
    | Sum f -> Field.{fname= sprintf "sum(%s)" f.fname; dtype= DInt}
    | Avg f -> Field.{fname= sprintf "avg(%s)" f.fname; dtype= DInt}
    | Min f -> Field.{fname= sprintf "min(%s)" f.fname; dtype= DInt}
    | Max f -> Field.{fname= sprintf "max(%s)" f.fname; dtype= DInt} )

let ok_exn = function Ok x -> x | Error e -> raise (TransformError e)

let rec to_schema_exn : t -> Schema.t =
 fun l ->
  match l.node with
  | Empty -> []
  | Int (_, m) | Bool (_, m) | String (_, m) | Null m -> [m.node.field]
  | ZipTuple ls | CrossTuple ls -> List.concat_map ls ~f:to_schema_exn
  | UnorderedList ls | OrderedList (ls, _) ->
...
let rec partition : PredCtx.Key.t -> Field.t -> t -> t =
  ...
  let p_crosstuple k f ls =
    let tbls, others =
      ...
      |> List.partition_tf ~f:(fun (_, l) ->
             match l.node with Table _ -> true | _ -> false )
    in
    match tbls with
    | [] -> cross_tuple (List.map others ~f:(fun (_, l) -> l))
    | [(i, {node= Table (m, {field= f; _}); _})] ->
        let elems =
          Map.map m ~f:(fun l ->
  ...
  let p_ziptuple k f ls =
    let all_have_f =
      List.for_all ls ~f:(fun l -> List.mem ~equal:Field.( = ) (to_schema_exn l) f)
    in
    if all_have_f then
      List.map ls ~f:(partition k f)
      |> List.fold_left
           ~init:(Map.empty (module ValueMap.Elem))
           ~f:(fun m l ->
             match l.node with
             | Table (m', {field= f'; _}) when Field.( = ) f f' ->
  ...
  let p_orderedlist k f ls ol =
    ...
    | ts ->
        ...
        Map.map merged_ts ~f:(fun ls ->
            List.fmerge ~cmp:Int.compare others ls
            |> List.map ~f:snd
           ...
           | Table (m_inner, _) ->
               Map.merge
                 ...
                 ~f:(fun ~key:_ -> function
                   | `Both (l, ls) ->
                       Some
                         (Map.merge l ls ~f:(fun ~key:_ -> function
                            | `Both (x, xs) -> Some (x :: xs)
                            | `Left x -> Some [x]
                            | `Right xs -> Some xs ))
  ...
  fun k f l ->
    if Schema.has_field (to_schema_exn l) f then
      ...
      | Table (_, {field= f'; _}) when Field.( = ) f f' -> l
...
let rec ntuples_exn : t -> int =
 fun l ->
  ...
  | Table (m, _) ->
      Map.data m |> List.map ~f:ntuples_exn
      |> List.all_equal ~sexp_of_t:[%sexp_of : int]
...
let rec flatten : t -> t =
 fun l ->
  ...
  | Table (m, x) ->
      ...
      if Map.for_all m' ~f:(fun v -> compare v empty = 0) then empty else table m' x
  | CrossTuple ls -> (
      let ls = List.map ls ~f:flatten in
      if List.exists ls ~f:(fun v -> compare v empty = 0) then empty
      else
        let ls =
          List.concat_map ls ~f:(fun l ->
              match l.node with CrossTuple ls' -> ls' | _ -> [l] )
        in
        match ls with [l] -> l | ls -> cross_tuple ls )
  | ZipTuple ls -> (
      let ls = List.map ls ~f:flatten in
      if List.exists ls ~f:(fun v -> compare v empty = 0) then empty
      else
        let ls =
          List.concat_map ls ~f:(fun l ->
              match l.node with ZipTuple ls' -> ls' | _ -> [l] )
        in
        match ls with [l] -> l | ls -> zip_tuple ls )
  | UnorderedList ls -> (
      let ls =
        ...
        |> List.concat_map ~f:(fun l ->
               ...
               | _ -> [l] )
      in
      match ls with [] -> empty | [l] -> l | ls -> unordered_list ls )
  | OrderedList (ls, x) -> (
      let ls =
        List.map ls ~f:flatten
        |> List.filter ~f:(fun v -> compare v empty <> 0)
        |> List.concat_map ~f:(fun l ->
               ...
               | _ -> [l] )
      in
      match ls with [] -> empty | [l] -> l | ls -> ordered_list ls x )
  | Grouping (ls, m) ->
      let ls = List.map ls ~f:(fun (k, v) -> (flatten k, flatten v)) in
      match ls with [] -> empty | ls -> grouping ls m

let order : Range.t -> Field.t -> [`Asc | `Desc] -> t -> t =
 fun k f o l ->
  let cmp =
    match o with
...
let project : Field.t list -> t -> t =
 fun fs l ->
  ...
  let rec project l =
    ...
    | CrossTuple ls ->
        cross_tuple
          (List.map ls ~f:project |> List.filter ~f:(fun l -> compare l empty <> 0))
    | ZipTuple ls ->
        zip_tuple
          (List.map ls ~f:project |> List.filter ~f:(fun l -> compare l empty <> 0))
...
let eq_join : Field.t -> Field.t -> t -> t -> t Lazy.t list =
 fun f1 f2 l1 l2 ->
  let t1, m1 =
    match (partition PredCtx.Key.Dummy f1 l1).node with
    | Table (t1, m1) -> (t1, m1)
    | _ -> failwith "Expected a table."
  in
  let t2, m2 =
    match (partition PredCtx.Key.Dummy f2 l2).node with
    | Table (t2, m2) -> (t2, m2)
    | _ -> failwith "Expected a table."
  in
  ...
  ; (* Materialized hash join on lhs *)
    lazy (cross_tuple [l1; table t2 {m2 with lookup= PredCtx.Key.Field f1}])
  ; (* Materialized hash join on rhs *)
    lazy (cross_tuple [l2; table t1 {m1 with lookup= PredCtx.Key.Field f2}]) ]

let accum : [`Gt | `Lt | `Ge | `Le] -> t -> t =
 fun dir l ->
  match l.node with
  | Table (m, x) ->
      let kv =
      ...
      let m, _ =
        List.fold_left kv
          ~init:(Map.empty (module ValueMap.Elem), [])
          ~f:(fun (m, vs) (k, v) ->
            let v' = match dir with `Gt | `Lt -> vs | `Ge | `Le -> v :: vs in
...
let tests =
  let open OUnit2 in
  let partition_tests =
    let f1 = Field.{fname= "f1"; dtype= DInt} in
    let f2 = Field.{fname= "f2"; dtype= DInt} in
    let r = {rname= "r"; fields= [f1; f2]} in
