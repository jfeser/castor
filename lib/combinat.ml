...
let m_partition_with_zeros : int -> int -> int Array.t Seq.t =
 fun n m ->
 ...
 fun a_init ->
  ...
  let rest_seq =
    Seq.unfold ~init:a_init ~f:(fun a ->
        ...
        let j = ref (n - 2) in
        while !j >= 0 && a.(!j) >= a.(!j + 1) do Caml.decr j done ;
        if !j < 0 then None
        else
          let l = ref (n - 1) in
          while a.(!j) >= a.(!l) do Caml.decr l done ;
          Array.swap a !j !l ;
          let k = ref (!j + 1) in
          let l = ref (n - 1) in
          while !k < !l do Array.swap a !k !l ; Caml.incr k ; Caml.decr l done ;
          Some (a, a) )
  in
  Seq.append init_seq rest_seq
