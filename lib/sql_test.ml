...
let%expect_test "foreach-1" =
  ...
  [%expect
    {| select t3."k", t4."g" from (select r."f" as k from r) as t3, lateral (select t2."g" from (select * from r where (r."f") = ("k")) as t2) as t4 order by (t3."k") |}]
...
let%expect_test "example2" =
  ...
  [%expect
    {| select t13."lp_k", t13."lc_k", t14."lp_counter", t14."lc_counter" from (select distinct * from (select t4."lp_id" as lp_k, t4."lc_id" as lc_k from (select * from (select "id" as lp_id from log_bench) as t2, (select "id" as lc_id from log_bench) as t3 where true) as t4) as t5) as t13, lateral (select t12."lp_counter", t12."lc_counter" from (select * from (select "counter" as lp_counter, "succ" as lp_succ from (select * from log_bench where (log_bench."id") = ("lp_k")) as t7) as t10, (select "counter" as lc_counter from (select * from log_bench where (log_bench."id") = ("lc_k")) as t9) as t11 where ((t10."lp_counter") < (t11."lc_counter")) and ((t11."lc_counter") < (t10."lp_succ"))) as t12) as t14 order by (t13."lp_k", t13."lc_k") |}]
