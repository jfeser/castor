...
module Make (Config : Config.S) : sig
  ...
  val eval : PredCtx.t -> (Field.t, Relation.t, Layout.t) Ralgebra0.t -> Tuple.t Seq.t
  val eval_partial : (Field.t, Relation.t, Layout.t) Ralgebra0.t -> (Field.t, Relation.t, Layout.t) Ralgebra0.t
end

module Make_relation (Config : Config.S_relation) : sig
  val eval_relation : Config.relation -> Tuple.t Seq.t
  val eval_pred : PredCtx.t -> Field.t Ralgebra0.pred -> primvalue
  val eval_aggregate : Tuple.t -> Field.t Ralgebra0.agg list -> Tuple.t Seq.t -> Tuple.t
  val eval_layout : PredCtx.t -> t -> Tuple.t Seq.t
  val eval_count : 'a Seq.t -> Tuple.t Seq.t
  ...
  val eval_concat : 'a Seq.t list -> 'a Seq.t
  val eval_agg : Field.t Ralgebra0.agg list -> Field.t List.t -> Tuple.t Seq.t -> Tuple.t Seq.t
  val eval : PredCtx.t -> (Field.t, Config.relation, Layout.t) Ralgebra0.t -> Tuple.t Seq.t
  val eval_partial : (Field.t, Config.relation, Layout.t) Ralgebra0.t -> (Field.t, Config.relation, Layout.t) Ralgebra0.t
end
