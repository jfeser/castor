...
let all_unref_at r r' =
  List.for_all (Schema.schema r) ~f:(fun n ->
      match Map.find r'.meta#meta#refs n with Some c -> not c | None -> false)

(** True if all fields emitted by r are unreferenced. *)
let all_unref r = all_unref_at r r

let project_list project no_project { l_keys = rk; l_values = rv } =
  let rk =
...
let project_open project no_project project_pred r =
  ...
  if all_unref r && not card_matters then dummy
    ...
    | DepJoin { d_lhs; d_rhs } ->
        if card_matters then A.dep_join (project d_lhs) (project d_rhs)
        else if
          (* `If one side of a join is unused then the join can be dropped. *)
          all_unref d_lhs
        then project d_rhs
        else if all_unref d_rhs then dummy
        else A.dep_join (project d_lhs) (project d_rhs)
