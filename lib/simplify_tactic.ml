...
module Make (C : Config.S) = struct
  ...
  let filter_const r =
    ...
    | Filter (`Bool false, _) -> Some A.empty
  ...
  let elim_depjoin r =
    ...
    let%bind { d_lhs; d_rhs } = to_depjoin r in
    match d_rhs.node with
    | AScalar s ->
        return @@ A.select (decr_select_scope [ (s.s_pred, s.s_name) ]) d_lhs
    | ATuple (rs, Cross) ->
        let%bind s =
          List.map rs ~f:(fun r ->
              match r.node with
          ...
          |> Option.all
        in
        return (A.select (decr_select_scope s) d_lhs)
    ...
    | Select (ps, { node = ATuple (rs, Cross); _ }) ->
        ...
        let ps' =
          concat_select (decr_select_scope ps') (schema d_lhs |> to_select_list)
        in
        return (A.select (decr_select_scope ps) (A.select ps' d_lhs))
    | Select (ps, { node = AScalar { s_pred = `Null None; _ }; _ }) ->
        return (A.select (decr_select_scope ps) d_lhs)
    | _ -> None
  ...
  let flatten_select r =
    ...
    let%bind ps', r'' = to_select r' in
    let%bind () =
      match (Abslayout.select_kind ps, Abslayout.select_kind ps') with
      | `Agg, `Agg -> None
      | _ -> Some ()
    in
    let ctx =
