...
let rec iter_expr (expr : _ Sql.expr) k =
  match expr with
  | Value _ | Param _ | Inserted _ | Column _ ->
      k expr
  | Fun (_, args) ->
      k expr ;
...
let attrs schema table =
  ...
  |> List.map ~f:(fun (cname, _, _) -> {Sql.Col_name.cname; tname= Some table})
...
let select_indexes queries =
  ...
  let eq_columns =
    Iter.map
      (function
        | Sql.Fun (`Eq, [Column lhs; Column rhs]) ->
            Iter.of_list [lhs; rhs]
        | Sql.Fun (`Eq, ([Column c; _] | [_; Column c])) ->
            Iter.singleton c
        | _ ->
            Iter.empty )
      preds
...
let idx_attrs db_schema idx =
  List.find_map_exn db_schema.indexes ~f:(fun (col, _, name, _) ->
      if [%equal: string] name idx then
        Some (attrs db_schema (Option.value_exn col.tname))
      else None )
...
let is_foreign_key_of db col tbl =
  match col_attrs db col with
  | Some (_, _, `Foreign_key tbl') ->
      [%equal: string] tbl tbl'
  | _ ->
      false

let foreign_key db col =
  match col_attrs db col with
  | Some (_, _, `Foreign_key t) ->
      Some t
  | _ ->
      None

let is_foreign_key db col =
  match col_attrs db col with Some (_, _, `Foreign_key _) -> true | _ -> false

let foreign_key_table db col =
  col_attrs db col
  |> Option.bind ~f:(function _, _, `Foreign_key tbl -> Some tbl | _ -> None)
