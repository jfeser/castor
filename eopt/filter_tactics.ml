...
let split_bound binder p =
  List.partition_tf (Pred.conjuncts p) ~f:(fun p' ->
      Set.overlaps (Free.pred_free p') (schema_set binder))
...
let select_kind g ps =
  Abslayout.select_kind @@ List.map ps ~f:(fun (p, n) -> (to_pred g p, n))

let hoist_filter_orderby g _ =
  let%bind { key; rel } = M.any_orderby g in
  let%map p, r = M.filter g rel in
  C.filter g p (C.order_by g key r)

let hoist_filter_groupby g _ =
  let%bind ps, key, r = M.any_groupby g in
  ...
  if
    invariant_support
      ...
      (to_pred g p)
  then Some (C.filter g p (C.group_by g ps key r))
  else None

let hoist_filter_filter g _ =
  let%bind p', r = M.any_filter g in
  let%map p, r = M.filter g r in
  C.filter g (C.binop g And p p') r

let hoist_filter_select g _ =
  let%bind ps, r = M.any_select g in
  ...
  | `Scalar ->
      if
        Tactics_util.select_contains
          (Free.pred_free (to_pred g p))
          ps (to_annot g r)
      then return (C.filter g p (C.select g ps r))
      else empty
  | `Agg -> empty

let hoist_filter_join_both g _ =
  let%bind { pred; r1; r2 } = M.any_join g in
  let%bind p1, r1 = M.filter g r1 in
  let%map p2, r2 = M.filter g r2 in
  C.filter g (C.and_ g [ p1; p2 ]) (C.join g pred r1 r2)

let hoist_filter_join_left g _ =
  let%bind { pred; r1; r2 } = M.any_join g in
  let%map p, r1 = M.filter g r1 in
  C.filter g p (C.join g pred r1 r2)
...
let hoist_filter_dedup g _ =
  let%bind r = M.any_dedup g in
  let%map p, r = M.filter g r in
  C.filter g p (C.dedup g r)

let hoist_filter_list g _ =
  let%bind l = M.any_list g in
  let%map p, r = M.filter g l.l_values in
  C.filter g
    (of_pred g (Pred.unscoped l.l_scope (to_pred g p)))
    (C.list g { l with l_values = r })

let hoist_filter_hashidx g _ =
  let%bind h = M.any_hashidx g in
  let%map p, r = M.filter g h.hi_values in
  let below, above = split_bound (to_annot g h.hi_keys) @@ to_pred g p in
  let above = List.map above ~f:(Pred.unscoped h.hi_scope) in
  C.filter g (C.and_ g @@ List.map ~f:(of_pred g) above)
  @@ C.hash_idx g
       {
         h with
         hi_values = C.filter g (C.and_ g @@ List.map ~f:(of_pred g) below) r;
       }

let hoist_filter_orderedidx g _ =
  let%bind o = M.any_orderedidx g in
  let%map p, r = M.filter g o.oi_values in
  let below, above = split_bound (to_annot g o.oi_keys) @@ to_pred g p in
  let above = List.map above ~f:(Pred.unscoped o.oi_scope) in
  C.filter g (C.and_ g @@ List.map ~f:(of_pred g) above)
  @@ C.ordered_idx g
       {
         o with
         oi_values = C.filter g (C.and_ g @@ List.map ~f:(of_pred g) below) r;
       }

let hoist_filter_agg g ctx =
  let%bind ps, r = M.any_select g in
  let%bind p, r = M.filter g r in
  match select_kind g ps with
  | `Scalar -> empty
  | `Agg ->
      if
        Tactics_util.select_contains
          (Set.diff (Free.pred_free (to_pred g p)) ctx#params)
          ps (to_annot g r)
      then return (C.filter g p (C.select g ps r))
      else empty

let hoist_filter_agg = Ops.create hoist_filter_agg "hoist-filter-agg"
...
let hoist_filter_extend_select g _ =
  let%bind ps, r = M.any_select g in
  let%bind p, r = M.filter g r in
  match select_kind g ps with
  | `Scalar ->
      let ext =
        let r = to_annot g r in
        Free.pred_free (to_pred g p)
        |> Set.to_list
        |> List.filter ~f:(fun f ->
               not
      ...
      in
      return @@ C.filter g p @@ C.select g (ps @ ext) r
  | `Agg -> empty

let hoist_filter_groupby r =
  let%bind ps, key, r = M.any_groupby g in
  | GroupBy (ps, key, r) ->
...
let split_filter_params r =
  ...
  if List.is_empty has_params || List.is_empty no_params then None
    ...
    Some
      (A.filter (Pred.conjoin has_params) @@ A.filter (Pred.conjoin no_params) r)
        ...
        |> List.partition_map ~f:(function
             | (`Binop (Binop.Gt, p1, p2) | `Binop (Lt, p2, p1)) as p ->
                 if is_candidate_key p1 r' && is_candidate_match p2 r' then
                   First (p2, (`Lt, p1))
                 else if is_candidate_key p2 r' && is_candidate_match p1 r' then
                   First (p1, (`Gt, p2))
                 else Second p
