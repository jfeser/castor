...
let all_values_approx params cost_conn ps r =
  if List.length ps = 1 then all_values_approx_2 cost_conn ps r
  else
    match all_values_approx_1 params ps r with
    | Ok r' -> Ok r'
    | Error _ -> all_values_approx_2 cost_conn ps r

let all_values params cost_conn ps r =
  match all_values_precise ps r with
  | Ok r' -> Ok r'
  | Error _ -> all_values_approx params cost_conn ps r
...
let rec disjoin =
  let open Ast in
  function
  | [] -> `Bool false | [ p ] -> p | p :: ps -> `Binop (Binop.Or, p, disjoin ps)

(** For a set of predicates, check whether more than one predicate is true at
     any time. *)
let all_disjoint conn ps r =
  let open Or_error.Let_syntax in
